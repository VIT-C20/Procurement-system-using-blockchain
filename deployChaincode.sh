...
presetup() {
    ...
    pushd ./artifacts/src/github.com/tendersys/go
...
VERSION="1"
CC_SRC_PATH="./artifacts/src/github.com/tendersys/go"
CC_NAME="tendersys"

packageChaincode() {
    rm -rf ${CC_NAME}.tar.gz
    setGlobalsForPeer0Org1
    ./artifacts/channel/bin/peer lifecycle chaincode package ${CC_NAME}.tar.gz \
...
installChaincode() {
    setGlobalsForPeer0Org1
    ./artifacts/channel/bin/peer lifecycle chaincode install ${CC_NAME}.tar.gz
...
queryInstalled() {
    setGlobalsForPeer0Org1
    ./artifacts/channel/bin/peer lifecycle chaincode queryinstalled >&log.txt
    cat log.txt
...
approveForMyOrg1() {
    setGlobalsForPeer0Org1
    # set -x
    ./artifacts/channel/bin/peer lifecycle chaincode approveformyorg -o localhost:7050 \
        --ordererTLSHostnameOverride orderer.tendersys.com --tls $CORE_PEER_TLS_ENABLED \
...
# --peerAddresses peer0.org1.tendersys.com:7051 --tlsRootCertFiles $PEER0_ORG1_CA --peerAddresses peer0.org2.tendersys.com:9051 --tlsRootCertFiles $PEER0_ORG2_CA
#--channel-config-policy Channel/Application/Admins
# --signature-policy "OR ('Org1MSP.peer','Org2MSP.peer')"

checkCommitReadyness() {
    setGlobalsForPeer0Org1
    ./artifacts/channel/bin/peer lifecycle chaincode checkcommitreadiness \
...
checkCommitReadyness() {
    ...
    setGlobalsForPeer0Org1
    ./artifacts/channel/bin/peer lifecycle chaincode checkcommitreadiness --channelID $CHANNEL_NAME \
...
commitChaincodeDefination() {
    setGlobalsForPeer0Org1
    ./artifacts/channel/bin/peer lifecycle chaincode commit -o localhost:7050 --ordererTLSHostnameOverride orderer.tendersys.com \
...
queryCommitted() {
    setGlobalsForPeer0Org1
    ./artifacts/channel/bin/peer lifecycle chaincode querycommitted --channelID $CHANNEL_NAME --name ${CC_NAME}
...
chaincodeInvokeInit() {
    setGlobalsForPeer0Org1
    ./artifacts/channel/bin/peer chaincode invoke -o localhost:7050 \
...
chaincodeInvoke() {
    # setGlobalsForPeer0Org1
    # peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer.tendersys.com \
...
chaincodeQuery() {
    ...
    # Query Car by Id
    ./artifacts/channel/bin/peer chaincode query -C $CHANNEL_NAME -n ${CC_NAME} -c '{"function": "queryTender","Args":["TENDER0"]}'
